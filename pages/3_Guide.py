...
md(
    "Every run's label spells out its configuration left-to-right, in plain language. Example: "
    "**10Δ · $50w · $40k · risk 50%≤$75k · SMA5 · Narrow Spread**\n"
    "- **10Δ** = the short-delta target (5Δ / 10Δ / 15Δ / 20Δ) — your main risk dial.\n"
    "- **$50w** = the spread width ($50).\n"
    "- **$40k** = starting capital.\n"
    "- **risk 50%≤$75k** = risk 50% of equity each week, capped at $75k (just **risk 50%** if uncapped).\n"
    "- **SMA5** = the trend filter (5-day SMA; e.g. **EMA9**, **SMA200**, or **no filter**).\n"
    "- **Narrow Spread** = the fill policy — **Narrow** (nearest narrower strike), **Exact** "
    "(only the exact width), **Widen**, or **Closest** (see §8 and AI Analysis §4).\n"
    "- A **tail** appears only for non-standard settings: withdrawals (`· WD $1,000/mo flr "
    "$42.5k`), a profit target / stop (`· PT 50%` / `· SL 50%`), a non-2% 1-DTE rule (`· 1DTE "
    "1.5%`), or non-standard commission/slippage (`· comm $0.70 · slip $0.02`).\n\n"
    "You rarely need to read it by hand — the **Risk profile** name (Conservative → "
...
md(
    ...
    "3. **Picks the long strike — the protective leg, set by a fill policy.** It buys a "
    "further-OTM put `width` below the short (e.g. $50 lower for a $50 spread) to cap the loss. "
    "When the *exact* strike `width` below isn't listed, a **fill policy** decides what to do — "
    "and the production runs use **Snap Narrower**: take the nearest available strike that only "
    "*narrows* the spread (never wider, so the exposure cap is always respected). Sized to the "
    "same dollar risk, a narrower fill simply means more contracts. Other policies are run for "
    "comparison — **Exact** (trade only the exact width, else skip the week), **Widen**, and "
    "**Closest** — and AI Analysis §4 contrasts Snap Narrower vs Exact. Narrow spreads ($10–$50) "
    "almost always find the exact strike, so the policy rarely matters; on wide spreads "
    "($100–$200) in the sparse early-period (2007–2011) chains it matters a lot — a \"$200\" "
    "spread could fill ~60-wide — which is why those years' drawdowns run deeper than a "
    "skip-the-week policy would show.\n"
